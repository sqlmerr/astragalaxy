...
pub(crate) fn router(state: AppState) -> Router<AppState> {
    ...
    Router::new()
        ...
        .route("/get-account-info", get(get_account_info).layer(auth_middleware))
}
...
async fn generate_ton_proof() -> Result<Json<GenerateTonProofPayload>, ApiError> {
    ...
    let payload = generate_ton_proof_payload(secret).await.map_err(|e| ApiError::TonError(e))?;
    Ok(Json(payload))
}
...
async fn check_proof(Json(body): Json<CheckProofPayload>) -> Result<Json<CheckTonProof>, ApiError> {
    ...
    let address = check_ton_proof(body, secret, domain).await.map_err(|e| ApiError::TonError(e))?;
